...
function CCshowMsg(msg) {
    ...
    $('body').after(strHtml)
    $('#loadMsg').animate({ opacity: '0.8' })
        $('#loadMsg').show();
    setTimeout(function () {
        $('#loadMsg').animate({ opacity: '0' }); 
    }, 2000)
    setTimeout(function () {
        $("#loadMsg").remove();
    }, 2500) 
...
function dateChaOneDay(star,end) {
    ...
    if (Math.abs(starDate.split('-')[0] - endDate.split('-')[0])>=1) { return false; }
    if (Math.abs(starDate.split('-')[1] - endDate.split('-')[1])>=1) { return false; }
    if (Math.abs(starDate.split('-')[2] - endDate.split('-')[2]) >= 2) { return false; }
 ...
 //   alert(parseInt(starTime.split(':')[2]) +'+'+ parseInt(endTime.split(':')[2]))
    ...
    } else if (Math.abs(starDate.split('-')[2] - endDate.split('-')[2]) == 0) { return true; }
