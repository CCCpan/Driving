...
//var requestURL = 'http://192.168.2.160:8080/';//宋
var requestURL = 'http://120.27.27.28:8899/';//云
//var requestURL = 'http://112.74.101.152:8899/';
//var requestURL = 'http://192.168.2.205:8081/';//周
var kangRequestURL = 'http://192.168.2.164:8081/';
...
//'2016-04-12T16:11:22.88'转换成正常时间
function changeData(dataStr){//'2016-04-12T16:11:22.88'
	...
	var newData = data.getUTCFullYear()+'-'+(data.getUTCMonth()+1)+'-'+data.getUTCDate()+' '+Hours+':'+Minutes+':'+Seconds;
	return newData;
...
function showNetWorkBanner(){
	var html = '';
	html += '<div id="network" style="width: 100%;height: 43px;position: fixed;top: 45px;left: 0px;z-index: 333; display: none;" onclick="netWork()">'
	   	 +'<img style="height: 100%;width: 100%;" src="images/network.png"/>'
	     +'</div>';
...
function netWork(){
	...
	var intent = new Intent(Settings.ACTION_WIFI_SETTINGS); 
 ...
 * */
	...
	//	console.log(forPageTitle+'+'+forPageUrl+'+'+publicPageID);
		...
		if(nowPage == forPageUrl){ 
			...
			template.show('pop-in', 180,function(){},'capture');									
		}else{ 
